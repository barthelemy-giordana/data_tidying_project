...
Installing (if needed) and loading packages needed to work on the project.
...
Downloading the data and setting work data frames.
...
Saving raw data in the directory "data/raw_data".
...
Wangling df2 to get a tidy data frame ("df_retail").
...
Wangling df1 to get a tidy data frame ("df_department").
...
df_total <- bind_rows(list(df_retail,df_department))
```

Making plots of each data frames.
